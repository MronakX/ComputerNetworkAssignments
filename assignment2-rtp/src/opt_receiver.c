...
int receiver(char *receiver_port, int window_size, char* file_name) {
  ...
  if(rtp_listen(receiver_fd, (struct sockaddr *)&sender, addr_len, 1) == -1)
  {
    perror("rtp_listen failed\n");
  ...
  printf("rtp accept successfully\n");
 // receive packet
 if ((recv_bytes = rtp_recvfrom(receiver_fd, (void *)buffer, sizeof(buffer), 0, 
      (struct sockaddr*)&sender, &addr_len, file_name)) < 0) {
         perror("receive error");
     }
     //buffer[recv_bytes] = '\0';

  rtp_close(receiver_fd);
  printf("receiver has made it to the END!\n");
  return 0;
}
