...
int rtp_sendto(int sockfd, const void *msg, int len, int flags, const struct sockaddr *to, 
    ...
    int isWindowComplete = 0;
    int isACKed[MAX_PACKET_NUM];
    memset(isACKed, 0, sizeof(isACKed));

    while(base != endnum)
    {
        printf("windowsize =%d, base = %d, start_seq_num = %d\n", windowsize, base, start_seq);
        isWindowComplete = 1;
        for(int i=0; i<windowsize; i++)
        {
            if(isACKed[i] == 0)
            {
                printf("ACK with seq %d has not received\n", i + base);
                isWindowComplete = 0;
                break;//not yet
            }
        }
        if(isWindowComplete == 1)//all done
        {
            base += windowsize;
            if(base >= endnum)
                break;
            memset(isACKed, 0, sizeof(isACKed));
            isWindowComplete = 0;
            if(windowsize > endnum - base)
            {
                windowsize = endnum - base;
            }
        ...
        while(isTimeout == 0)//超时 无任何收取
        {
            ...
            for(int i = base-start_seq; i<=nextseqnum-1-start_seq; i++)
            {
                //sendto(i);
                printf("i+start_seq-base=%d\n", i+start_seq-base);
                if(isACKed[i+start_seq-base] == 0)
                {
                    printf("seq %d not ACK\n", i);
                    char buffer[BUFFER_SIZE];
                    rtp_header_t* rtp = (rtp_header_t*)buffer;
                    rtp->length = strlen(packets[i]);
                    rtp->checksum = 0;
                    rtp->seq_num = i+start_seq;
                    rtp->type = RTP_DATA;
        ...
        while(isTimeout == 1)
        {
            ...
            if(isChecksumLegal == 1 && ACK_seq >= base && ACK_seq < base + windowsize)
            {
                isACKed[ACK_seq - base] = 1;//packet with ACK_seq is ACKed.
                isTimeout = timeout(sockfd);
            }
            else if(isChecksumLegal == -1 || ACK_seq < base || ACK_seq >= base + windowsize)
            {
...
int rtp_recvfrom(int sockfd, void *buf, int len, int flags,  struct sockaddr *from, 
    ...
    int windowsize = rcb->window_size;
    int window_buffer_length[MAX_PACKET_NUM];
    char *window_buffer[MAX_PACKET_NUM][MAXLINE + 1];


    int isACKed[MAX_PACKET_NUM];
    int isWindowComplete = 0;

    memset(window_buffer_length, 0, sizeof(window_buffer_length));
    ...
    while(1)
    {
        ...
        for(int i=0; i<windowsize; i++)
        {
            ...
            {
                isWindowComplete = 0;
                printf("packet with num %d has not received!\n", i + expect_seq_num);
                break;
            }
        }
        if(isWindowComplete == 1)
        {
            for(int i=0; i<windowsize; i++)
            {
                if(write(write_fd, window_buffer[i], window_buffer_length[i]) != -1)
                printf("write success\n");
            }

            expect_seq_num += windowsize;
        ...
        printf("rtp->seq_num = %d, expect_seq_num = %d\n", rtp->seq_num, expect_seq_num);
        if((isChecksumEqual == 1) && rtp->type == RTP_END)
        {
            ...
            break;
                ...
                && (rtp->seq_num < expect_seq_num + windowsize))
        {
            //expect_seq_num++;
            ...
            //    printf("write success\n");
            printf("rtp->seq_num = %d, expect_seq_num = %d\n", rtp->seq_num, expect_seq_num);
            isACKed[rtp->seq_num - expect_seq_num] = 1;
            sendback_ACK(sockfd, rtp, buffer, rtp->seq_num, from, fromlen);
            //printf("bp1\n");
            memcpy(window_buffer[rtp->seq_num - expect_seq_num], (void*)buffer+sizeof(rtp_header_t), rtp->length);
            //printf("bp2");
            window_buffer_length[rtp->seq_num - expect_seq_num] = rtp->length;
            //printf("bp3!\n");
        }
    }
